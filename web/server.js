...
async function loadStats(shop, token, days) {
  ...
  const PQ = "query($n:Int!,$q:String,$after:String){ orders(first:$n, reverse:true, query:$q, after:$after){ pageInfo{ hasNextPage endCursor } edges{ node{ lineItems(first:50){ edges{ node{ title quantity discountedTotalSet{ shopMoney{ amount currencyCode } } originalTotalSet{ shopMoney{ amount currencyCode } } discountAllocations{ allocatedAmountSet{ shopMoney{ amount currencyCode } } } customAttributes{ key value } } } } } } } }";
  let orders = [];
  { let after = null, pages = 0;
    while (true) {
      const j = await gql(shop, token, PQ, { n: 250, q: "created_at:>=" + since, after });
      if (j.errors)
        return { error: JSON.stringify(j.errors), pdp: { total: 0, revenue: 0, items: [] }, cart_drawer: { total: 0, revenue: 0, items: [] } };
